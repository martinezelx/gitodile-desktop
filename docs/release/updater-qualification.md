...
After both private signing runs succeed, dispatch
`Controlled updater qualification bundle` with their numeric run IDs. The
workflow verifies both complete signed matrices and creates one private Actions
...
real target. Record the OS version, architecture, install path class and whether
hardware or a VM was used. Before requesting B, create identifiable settings,
...
sizes/hashes, updater public key ID, OS trust identity, and Apple notarization
...
a pass merely because user data survived. macOS additionally remains blocked
until the app-bundle replacement concern has an independently reviewed
mitigation and a real target report hash.

## Evidence registry and promotion gate

[`update-target-qualifications.json`](update-target-qualifications.json) uses
schema version 2 and stays deny-by-default. Do not add placeholder, mocked,
...
- independent replacement-safety evidence for each macOS target.

Production approval is separate and requires structured working-name clearance
plus a publisher qualification report proving full-matrix failure, retry,
immutable assets, preview/stable ordering and anonymous access. The publisher
rejects the old shallow schema, duplicate/cross-target claims, incomplete
failure matrices and missing macOS replacement evidence.

Keep a target `qualification_required` whenever its evidence is absent or
ambiguous. Keep `productionPromotion.enabled` false until all four exact targets
and the external approval are real. Run `pnpm run check` and then
...
signed A/B matrix or installed-platform evidence exists. All four targets and
production promotion remain disabled; this document must not be read as a
